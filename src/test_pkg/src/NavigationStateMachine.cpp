      ...
      current_rotated_angle_(0.0f)
{
    ...
    simulation_service_client_ = nh_.serviceClient<test_pkg::Service>("/task_service");
...
void NavigationStateMachine::handleWaitingSimulation() {
    static bool first_enter = true;
    static ros::Time wait_start_time;
    test_pkg::Service srv;
    
    if (first_enter) {
        speak("开始等待仿真任务");
        
        // 发布任务类型
        srv.request.task_type = current_task_;
        ROS_INFO("已发布任务类型: %s", current_task_.c_str());
    ...
    // ✅ 等待仿真返回结果
    double wait_time = (ros::Time::now() - wait_start_time).toSec();
    ROS_INFO_THROTTLE(2, "[WAITING_SIMULATION] 等待仿真返回结果... 已等待: %.1f 秒", wait_time);
    
    if (simulation_service_client_.call(srv)) {
        simulationCallback(srv.response.result);
...
bool NavigationStateMachine::callQRService() {
    ...
    if (qr_service_client_.call(srv)) {
        if (srv.response.success) {
            ...
            task_srv.request.task_type = current_task_;
...
void NavigationStateMachine::simulationCallback(const std::string msg) {
    simulation_result_ = msg;
...
bool NavigationStateMachine::callSimulationService() {
    try {
        ...
        test_pkg::Service srv;   // 创建服务请求对象
        
        srv.request.task_type = current_task_; // 传递任务类型
        
        ROS_INFO("任务类型: %s", current_task_.c_str());
        
        // 调用服务
        if (simulation_service_client_.call(srv)) {
            simulation_result_ = srv.response.result;
            task_flags_.simulation_received = true;
            ROS_INFO("返回结果: %s", simulation_result_.c_str());
            ...
            return true;
                ...
                ROS_WARN("结果返回失败: %s", srv.response.result.c_str());
                return false;
        }
    }
    catch (const std::exception& e) {
