...
int main(int argc, char** argv) {
    ...
    // 函数声明
    bool demoHandle(test_pkg::Service::Request& req, test_pkg::Service::Response& res);
    ...
    // 创建服务
    ros::ServiceServer task_service_server_ = nh.advertiseService("/task_service", demoHandle);
    ...
    while (ros::ok()) {
        ...
        if (input == "q" || input == "Q") {
            ...
            srv.response.result = msg.data;
            ROS_INFO("已发送仿真任务结果: %s房间", srv.response.result.c_str());
...
bool demoHandle(test_pkg::Service::Request &req, test_pkg::Service::Response &res) {
    ...
    ROS_INFO("收到服务请求，任务类型: %s", req.task_type.c_str());
    std::cout << "输入仿真结果房间号 (A/B/C): ";
    std::getline(std::cin, input);
    
    if (input == "A" || input == "a") msg = "A";
    else if (input == "B" || input == "b") msg = "B";
    else if (input == "C" || input == "c") msg = "C";
    else {
        ROS_WARN("无效房间号，使用默认: A");
        msg = "A";
    }
    
    srv.response.result = msg;
    ROS_INFO("已发送仿真任务结果: %s房间", msg.c_str());
