...
SimulationStateMachine::SimulationStateMachine(ros::NodeHandle& nh) 
    ...
    // 创建服务
    task_service_server_ = nh_.advertiseService("/task_service", &SimulationStateMachine::handleTask, this);
...
void SimulationStateMachine::startCallback(const std::string target_task_) {
...
void SimulationStateMachine::publishFinalResult() {
    ...
    } else if (current_state_ == SimulationState::ERROR){
        result_message = "仿真任务执行失败";
...
bool SimulationStateMachine::handleTask(test_pkg::Service::Request  &req, test_pkg::Service::Response &res) {
    startCallback(req.task_type);
    res.result = final_result_;
    ROS_INFO("发布仿真结果给客户端: %s", final_result_.c_str());
    return true;
}
